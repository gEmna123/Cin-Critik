...
$notecritique = isset($_POST['notecritique']) ? (float)$_POST['notecritique'] : null;	if (!isset($_SESSION['user_id'])) {
    ...
    $query = "INSERT INTO Critique ( contenucritique,notecritique, idUtilisateur, idoeuvre)
              VALUES ( '$contenu',$notecritique ,  $idUtilisateur, $idoeuvre)";
    // Exécution de la requête
       if (pg_query($conn, $query) === FALSE) {
           echo "Erreur lors de l'ajout de la critique : " . pg_last_error($conn);
       }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajouter_evenement'])) {
    ...
    $type = pg_escape_string($conn, $_POST['type_evenement']);

    $description = pg_escape_string($conn, $_POST['description_evenement']);
    

    if (!empty($nomEvenement) && !empty($dateEvenement) && !empty($description)) {
        $query = "INSERT INTO Evenement (nomEvenement, dateEvenement, descriptionEvenement) 
                  VALUES ('$nomEvenement', '$dateEvenement', '$description')";
        $query1 = "INSERT INTO typeevenement (libelletypeevenement) VALUES ('$type')";
        ...
        if ($result && $result1) {
            ...
            echo "<p style='color: red;'>Erreur lors de l'insertion : " . pg_last_error($conn) . "</p>";
        }
    } else {
...
$critiques_query = "
    SELECT c.*, o.titreoeuvre 
    FROM Critique c
    JOIN Oeuvre o ON c.idoeuvre = o.idoeuvre
    ORDER BY c.idcritique
";
...
<form method="POST">
    ...
    <label>Note (sur 5) :
        <input type="number" name="notecritique" min="0" max="5" step="0.1" required>
    </label><br>

    <label>Œuvre à critiquer :
        <select name="idoeuvre" required>
        ...
        <?php if ($critiques): ?>
            <?php foreach ($critiques as $critique): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($critique['titreoeuvre']) ?></td>
                    <td><?= htmlspecialchars($critique['contenucritique']) ?></td>
    ...
    <form method="POST">
        ...
        <label>Type <input type="text" name="type_evenement" required></label><br>

        <label>description: <input type="text" name="description_evenement" required></label><br>
    ...
    <?php if ($evenements): ?>
        <?php foreach ($evenements as $evenement): ?>
            <tr>
                ...
                <td>
                    <a href="?supprimer_evenement=<?= $evenement['idevenement'] ?>" 
        ...
        <?php if ($oeuvres): ?>
            <?php foreach ($oeuvres as $oeuvre): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($oeuvre['titreoeuvre']) ?></td>
